...
def process(rdd):
	if not rdd.isEmpty():
		...
		for cls_name, cls in perc_models.items():
...
if __name__ == '__main__':
	...
	for cls_name in perc_models :
		perc_models[cls_name]["model"] = joblib.load(f"{cls_name}.pkl")
	...
	with open("score1.txt",'w') as file1:
		file1.write(str(perc_models))
	...
	len_list = [i for i in range (1, (len(x_lists[0]["PERC1"])+1))]

	for i in range (0, len(x_lists)):
		plot_graphs(x_lists[i], name_list[i])
	plt.show()
