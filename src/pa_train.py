...
def process(time, rdd):
	if not rdd.isEmpty():
		...
		for cls_name, cls in pa_classifiers.items():
...
if __name__ == '__main__':
	...
	sc = SparkContext("local[2]", "PassiveAggressiveClassifier")
	...
	# create PA Classifiers
	pa_classifiers = {
		"PA1" : PassiveAggressiveClassifier(),
		"PA2" : PassiveAggressiveClassifier(loss='hinge'),
		"PA3" : PassiveAggressiveClassifier(shuffle=True)
	}
	...
	ssc.awaitTermination(timeout=60*30)
